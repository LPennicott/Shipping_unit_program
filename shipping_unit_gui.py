...
def view_shipments():
	...
	[sg.Cancel(), sg.Button('Release', key = 'release'), sg.Button('Delete', key = 'delete')]]
	...
	if event == 'release':
		if values['table']:
			...
			layout_confirm = [
				[sg.Text(f'Please confirm you want to release the following shipments: {on_hands}')],
	...
	if event == 'delete':
		if values['table']:
			...
			layout_confirm = [
				[sg.Text(f'Please confirm you want to delete the following shipments: {on_hands}')],
				[sg.Button('Yes', key = 'yes')], [sg.Button('No', key = 'no')]
			]
...
def update_a_shipment(onhand):
	...
	layout = [
		...
		[sg.Text('Client:', font = ('Comic', 15)), sg.InputText(f'{unit.client}', key = 'client')],
		[sg.Text('Width:', font = ('Comic', 15)), sg.InputText(f'{unit.width}', key = 'width')],
		[sg.Text('Length:', font = ('Comic', 15)), sg.InputText(f'{unit.length}', key = 'length')],
		[sg.Text('Height:', font = ('Comic', 15)), sg.InputText(f'{unit.height}', key = 'height')],
		[sg.Text('Gross Weight:', font = ('Comic', 15)), sg.InputText(f'{unit.gross_weight}', key = 'gw')],
		[sg.Text('Purpose:', font = ('Comic', 15)), sg.InputText(f'{unit.purpose}', key = 'purpose')],
	...
	window = sg.Window(f'Update Shipment: {unit.on_hand_number}', layout, resizable = True)
	event, values = window.read()
	window.close()
...
def consolidation():
	...
	[sg.Cancel(), sg.Button('Consol', key = 'consol'), sg.Button('Delete', key = 'delete')]]
	...
	#assign on-hands a MAWB number (or consol date), total weight and shipping unit count.
...
def report():
	...
	ezgmail.init()
	ezgmail.send('lpennicott3@gmail.com', 'Shipping Unit Program!', str(results[0][0]))