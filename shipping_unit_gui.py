...
def save_new_shipment():
    ...
    layout = [
        ...
        [sg.Text('Gross Weight', size=(15, 1)),
            sg.InputText(key='Gross Weight')],
...
def view_shipments():
    ...
    layout = [[sg.Table(values=result,
        ...
        [sg.Cancel(), sg.Button('Release', key='release'),
    ...
    if event == 'release':
        if values['table']:
            ...
            layout_confirm = [
                [sg.Text(f'Please confirm you want to\
...
def export_to_csv():
    ...
    with open(f'{from_date} to {to_date} consol.csv',
...
def report():
    ...
    ezgmail.init()
    ezgmail.send('lpennicott3@gmail.com',
                 'Shipping Unit Program!',
                 str(results[0]))
